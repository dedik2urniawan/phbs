-- =========================================================
-- PATCH 024: SCHEDULE TOGGLE RLS & SECURITY HARDENING
-- Tujuan:
-- 1. Menambahkan RLS Policy SELECT & UPDATE pada ref_puskesmas
--    sehingga Superadmin bisa mengubah is_active secara sah.
-- 2. FK Safety: Mencegah Foreign Key Violation pada surveys.kader_id di RPC composite.
-- 3. Security Hardening: SET search_path = public pada fungsi SECURITY DEFINER.
-- =========================================================

BEGIN;

-- 1. Enable & Tambahkan RLS Policy pada ref_puskesmas
ALTER TABLE public.ref_puskesmas ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "ref_puskesmas_select_all" ON public.ref_puskesmas;
DROP POLICY IF EXISTS "ref_puskesmas_update_superadmin" ON public.ref_puskesmas;

CREATE POLICY "ref_puskesmas_select_all" 
...
-- 2. Security Hardening pada Fungsi Database (P3 Security Linter)
...
BEGIN
    ...
    END IF;
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'get_phbs_analytics') THEN
        ALTER FUNCTION public.get_phbs_analytics(uuid) SET search_path = public;
    END IF;
...
-- Batasi hak akses eksekusi fungsi
...
-- 3. Perbarui RPC sync_offline_composite agar Kebal dari FK Violation kader_id
...
BEGIN
    ...
    IF p_survey IS NOT NULL AND (p_survey->>'id') IS NOT NULL THEN
        ...
        INSERT INTO public.surveys (
            ...
            (p_survey->>'i13_pengunjung_posyandu')::boolean,
